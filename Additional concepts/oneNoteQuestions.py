# #1. Write a Python program to extract the first and last characters of a string entered by the user. 

# userString = input("Enter a sentence: ")

# print(userString[0])
# print(userString[-1])

# #2. Write a Python program to reverse a string entered by the user. 

# unoReverse = input("Enter a sentence: ")
# reverseString = ""

# for character in unoReverse:
#     reverseString = character + reverseString
# print(reverseString)


#3. Write a Python program to check if a string is a palindrome (reads the same forward and backward). 

#4. Write a Python program to count the number of vowels in a string. 

#5. Write a Python program to replace all spaces in a string with underscores (_). 

 

# #6. Generate and print one random integer from 1 to 6. 
# import random

# myRandomNumber = random.randint(1, 6)
# print(myRandomNumber)


# #7. Roll two dice (1–6) and print both values and their total using + string concatenation. 
# import random

# myRandomNumber = random.randint(1, 6)
# print(myRandomNumber)

# myRandomNumber2 = random.randint(1, 6)
# print(myRandomNumber2) 
# print(myRandomNumber + myRandomNumber2)

#ORRR
# import random
# firstDice = random.randint(1, 6)
# secondDice = random.randint(1, 6)
# print(str(firstDice) + " + " + str(secondDice) + " =")
# print(str(firstDice + secondDice))

#8. Given the list below, use a random integer to select a valid index and print the item at that index. 
#   animals = ["otter", "red squirrel", "pine marten", "seal", "oystercatcher"] 
